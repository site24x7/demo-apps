"""FastAPI integration — one-liner setup.
...
from .middleware import ChaosASGIMiddleware

if TYPE_CHECKING:
    from fastapi import FastAPI
    from sqlalchemy.engine import Engine as SAEngine

logger = logging.getLogger("site24x7_chaos.fastapi")


def _get_fastapi_version() -> str:
    """Best-effort FastAPI version detection."""
    try:
        import fastapi
...
def init_chaos(
    ...
    *,
    ...
    """Initialize the Chaos SDK for a FastAPI application.

    This single call:

    1. Creates and starts a :class:`ChaosEngine` (config file watcher).
    2. Adds :class:`ChaosASGIMiddleware` (inbound HTTP faults).
    ...
    engine = ChaosEngine(
        ...
        framework="fastapi",
        framework_version=_get_fastapi_version(),
    )
    engine.start()
    ...
    # 1. Inbound HTTP middleware (ASGI)
    ...
    logger.info("Chaos SDK initialized for FastAPI app '%s'", engine.app_name)
    return engine
