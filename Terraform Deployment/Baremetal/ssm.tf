    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "# Set root password safely (works for fresh + existing installs)",
            "echo 'Configuring MySQL users...'",
            "sudo mysql <<'MYSQL_EOF'",
            "ALTER USER IF EXISTS 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'ZylkerKart@2024';",
            ...
            "# Wait again (important after restart)",
            "for i in {1..30}; do",
            "  if mysqladmin -u root -p'ZylkerKart@2024' ping --silent; then",
            "    echo 'MySQL authenticated access ready'; break;",
            "  fi",
            "  sleep 2",
            "done",

            "# Tune MySQL (safe execution)",
            "mysql -u root -p'ZylkerKart@2024' -e \"SET GLOBAL innodb_flush_log_at_trx_commit = 0; SET GLOBAL max_allowed_packet = 67108864; SET GLOBAL innodb_change_buffering = 'all';\" || true",
    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "# Install Python 3.11",
            "apt-get update -y",
            "apt-get install -y curl ca-certificates python3.11 python3.11-venv python3-pip",
            "",
            "# Download artifact",
            "cd /tmp",
