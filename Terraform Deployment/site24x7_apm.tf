...
#    Polls every 30s, up to 5 minutes (10 attempts)
# ──────────────────────────────────────────────
resource "null_resource" "wait_for_apm_agents" {
  ...
  provisioner "local-exec" {
    command = "powershell -NoProfile -File ${abspath(path.module)}\\scripts\\wait_for_apm_registration.ps1 -AppNamePrefix ${var.apm_app_name_prefix} -ExpectedAppCount ${var.expected_app_count} -MaxAttempts 7 -DelaySec 30"
  }
...
#     Replaced each run (timestamp trigger) so
#     the local state file always reflects the
#     latest APM data. No destroy provisioner —
#     a replace cycle won't call Site24x7 DELETE.
# ──────────────────────────────────────────────
resource "terraform_data" "apm_state_refresh" {
  ...
  # ── APPLY: fetch and overwrite the local state file ──
  provisioner "local-exec" {
    command = "powershell -NoProfile -File ${abspath(path.module)}\\scripts\\fetch_and_store_apm.ps1 -OutputFile ${abspath(path.module)}\\apm_monitors_state.json -AppNamePrefix ${var.apm_app_name_prefix} -ExpectedAppCount ${var.expected_app_count}"
  }
}

# ──────────────────────────────────────────────
# 5b. DELETE APM monitors only on terraform destroy
#     No triggers_replace — this resource is created
#     once and only destroyed during a full destroy,
#     which is when the real Site24x7 API DELETE runs.
# ──────────────────────────────────────────────
resource "terraform_data" "apm_monitor_cleanup" {
  ...
  # ── DESTROY: delete APM monitors from Site24x7 ──
  provisioner "local-exec" {
    when    = destroy
    command = "powershell -NoProfile -File ${self.input.script_dir}\\delete_apm_monitors.ps1 -InputFile ${self.input.state_file} -AppNamePrefix ${self.input.app_prefix}"
  }
}
